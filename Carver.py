...
def main(argv):
    try:
        ...
        rwgroup.add_argument('-r', '--read', help='The file to read from FAT32 volume.', action='store_true',
                             required=False)
        rwgroup.add_argument('-s', '--search', help='Search for JPG/BMP.', action='store_true', required=False)
        ...
        if not (search):
            ...
            status, error = SearchData(volume)
            if (status):
